...
class CVEClient(object):
    ...
    def cve_id(self, cve: str) -> Optional[CVE]:
        ...
        if os.path.exists(
            f"{CACHE_PATH}/cve/{cve.upper()}"
        ):  # TMP - will be helpful for gathering stats for ref tags usage
            logger.info(f"clients: cve: Using cached data for {cve}.")
            return self._parse_output(self.load_cve_from_cache(cve))
        elif not os.path.exists(f"{CACHE_PATH}/cve"):
            os.makedirs(f"{CACHE_PATH}/cve")
